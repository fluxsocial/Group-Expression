...
use crate::{GroupExpression, GroupExpressionData};
...
impl TryFrom<CreateExpression> for GroupExpression {
    type Error = WasmError;

    fn try_from(content: CreateExpression) -> Result<Self, Self::Error> {
        let expression: GroupExpressionData = serde_json::from_str(&content.data)
