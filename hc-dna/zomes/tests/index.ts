...
orchestrator.registerScenario("create and get public expression", async (s, t) => {
  ...
  const create_exp = await alice_happ.cells[0].call("group-expression", "create_public_expression", 
    {data: JSON.stringify({description: "My group description", name: "My group"}), author: {did: "did://alice", name: null, email: null}, timestamp: new Date().toISOString(), proof: {key: "key", signature: "sig"}})
